...
- **Drain a closed item into the workstream's dated `done/` ledger.** When an item's completion is recorded and
  ...
  **Name that file for what the work was about, at a glance.** `<date>-the-lipika-extraction.md`, where a
  generic `<date>-landed-and-closed.md` leaves every closed ledger in a workstream looking alike and a reader
  opening all of them — measured: one workstream accumulated four in three days. Take the theme from the items
  you are draining: the effort's name, the PR's subject, the task you drained out of. **A rough name beats a
  generic one**, so take the first that fits and move on. Better still, append to an existing `done/` doc whose
...
that licensed it; the `lipika marker-licence-check` output; every line you left alone and why (missing evidence,
